...
Toy 排行榜昵称不属于静态 UI 文案，内容直到 SDK 返回时才知道，因此不加入这个构建期子集。Web 端统一由 `PalWebTextRenderer` 使用浏览器 Canvas 和系统中文字体生成 400 字重的 16px 字形，再以 alpha 128 把抗锯齿透明度二值化为硬边纹理；桌面视觉回归使用 Godot 系统 fallback。这样任意玩家昵称不需要提前写入点阵图集，在 320×200 画面整数放大后也不会出现灰边模糊或笔画粘连。
