...
### [BF-022] 仙灵岛洗澡动画被黑色渐隐遮罩覆盖

- **来源**: 用户试玩反馈
- **关联需求**: M2–M3 仙灵岛主线过场表现
- **问题描述**: 原版洗澡剧情并非独立 RNG，而是先用 `0050` 渐隐，再逐帧切换 EventObject 209 的 MGO 339 第 8–16 帧。SDLPal 的 `fNeedToFadeIn` 会在下一次 `PAL_MakeScene` 自动渐显；Godot 版只在显式 `0005` 重绘、脚本结束或切场景时消费该状态，导致帧等待期间动画已经播放，顶层黑色遮罩却一直没有退场。
- **涉及文件**:
  - `src/world/map_explorer.gd`
  - `tests/run_tests.gd`
  - `tests/run_local_early_mainline_test.gd`
  - `docs/SCRIPT_VM.md`
- **修复内容**: 将自动渐显挂到统一世界刷新完成点；活动渐隐结束且没有待加载场景时，首次普通帧刷新也会消费 `0050` 状态并渐显。合成回归覆盖活动渐隐、待切场景和普通世界刷新三种边界，真实资源回归确认 EventObject 209 使用 MGO 339 且具备完整第 8–16 帧。
- **状态**: ✅ 已修复
