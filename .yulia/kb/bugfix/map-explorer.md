...
- **来源**: 用户试玩反馈
- **关联需求**: M5 白河村主线完整试玩
- **问题描述**: 河边使用钓竿取得鲤鱼、前山路捕鹿后放鹿时，脚本会正常执行并允许继续推进，但整段人物／鹿动作都被纯黑画面遮住。两个入口都先用 `0050` 渐隐，再以 `0009` 的逐帧游戏更新播放动作；正式 `MapExplorer` 只在 `redraw_requested` 与镜头移动回调中消费待渐显标记，普通脚本帧虽同步了 `PalTileMapWorld`，却没有像 SDLPal 的 `PAL_MakeScene()` 一样开始渐显。
- **涉及文件**:
  - `src/world/map_explorer.gd`
  - `tests/run_local_baihe_hunting_runtime_test.gd`
  - `docs/DEVELOPMENT_WORKFLOW.md`
- **修复内容**: 自动脚本帧改变世界后，在正式 TileMap 同步完成的同一更新周期消费 `0050` 留下的待渐显标记。新增真实资源运行回归，实例化正式 `MapExplorer + TileMapLayer + PalTileMapWorld`，分别执行捕鱼入口 `14358` 与放鹿入口 `14840`，确认捕鱼动作帧 `0–26` 和鹿的 `0009 12` 自动移动都在黑幕揭开后可见；带窗口运行会检查实际非黑像素并保存两张 Git 忽略截图。
- **状态**: ✅ 已修复
