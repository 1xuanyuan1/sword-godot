...
### [BF-047] 飞龙探云手失败弹窗缺少“败”字

- **来源**: 用户试玩反馈
- **关联需求**: M4 经典战斗偷窃反馈
- **问题描述**: 新增的“偷取失败”弹窗使用简体“败”，但导入的原版 Big5 点阵字库只包含繁体“敗”。共享简繁兼容表没有这组映射，导致弹窗最后一个字按缺失字形处理，画面只显示“偷取失”。
- **涉及文件**:
  - `src/ui/pal_classic_font.gd`
  - `tests/run_tests.gd`
  - `tests/run_local_battle_steal_feedback_test.gd`
  - `.yulia/kb/bugfix/battle.md`
- **修复内容**: 在共享经典字体兼容层增加“败 → 敗”映射，使所有简体 UI 文案复用原版 16×15 点阵字形；合成回归验证别名解析，真实 OpenGL 战斗回归同时验证“偷取失败”每个字符都存在于经典字形表，并固定“败”必须与“敗”使用同一字形坐标。
- **状态**: ✅ 已修复
...
### [BF-045] 后续队员主指令阶段按 ESC 无法退回上一名队员

- **来源**: 用户试玩反馈
- **关联需求**: M4 经典战斗指令选择
- **问题描述**: 多人队伍中，李逍遥等前一名队员提交指令、轮到赵灵儿等后续队员的主指令页后，按 ESC 不会撤销前一条指令或退回前一名队员。表现层只能退出子菜单，控制器没有对应 SDLPal 经典模式的指令回退与消耗品预占释放能力。
- **涉及文件**:
  - `src/battle/pal_battle_controller.gd`
  - `src/battle/pal_battle_preview.gd`
  - `tests/run_battle_logic_tests.gd`
  - `tests/run_local_battle_command_cancel_test.gd`
  - `.yulia/kb/bugfix/battle.md`
- **修复内容**: 控制器新增经典指令撤销：从当前队员向前寻找最近一名可手动下令的队员，跳过倒下、混乱、睡眠和定身角色，清理路径上已分配的指令，并释放被撤销的消耗品或投掷物库存预占。战斗主指令页的 ESC 优先调用该撤销；目标、仙术、物品等子页仍按原层级逐层返回。合成回归覆盖双人回退、首人边界、消耗品预占释放和跳过睡眠队员；真实 OpenGL 回归检查李逍遥／赵灵儿指令流、当前队员标记像素变化及截图。
- **状态**: ✅ 已修复
