...
### [BF-057] Toy 排行榜昵称锐化后笔画过粗粘连

- **来源**: 用户检查 Web 字重对照反馈
- **关联需求**: M5 Toy Web 发布质量
- **问题描述**: BF-056 首版采用 600 字重并以 alpha 96 保留较多边缘像素，虽然去除了灰边，但中文相邻笔画在 16px 字形中明显粘连；整数放大后形成粗重马赛克，视觉效果比原先的抗锯齿模糊更差。
- **涉及文件**:
  - `src/ui/pal_web_text_renderer.gd`
  - `tests/run_tests.gd`
  - `assets/ui/PAL_SIMPLIFIED_FONT.md`
  ...
  - `docs/PROJECT_STRUCTURE.md`
  - `.yulia/kb/bugfix/game-menu.md`
- **修复内容**: 使用真实浏览器对比 400／500／600 字重和不同透明度阈值，最终改为 400 字重、16px、alpha 128；细化笔画并减少边缘保留，仍输出纯 0／255 alpha。合成测试改为验证默认阈值 127／128 的边界，防止配置回退到过粗方案。
- **状态**: ✅ 已修复
