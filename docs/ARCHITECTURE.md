...
`PalContentDatabase` 读取敌人、仙术、战场、站位、升级阈值/习得仙术规则、官方 UI、双方战斗 Sprite 和 FIRE 仙术 Sprite，这些都是只读内容。`PalBattleController` 从这些定义创建单场敌人状态，收集全队攻击/防御/仙术指令，生成经典身法队列，并把 MP、伤害、恢复、经验、金钱和升级结算写入正式状态后返回与画面无关的 `ActionResult/RewardResult`。角色跨战斗的体力、真气、主经验、等级、成长属性和已学仙术继续由 `GameSession` 持有；`PalBattlePreview` 根据结果播放接近、攻击、施法、受击和归位动画，`PalBattleUI` 只绘制官方状态框、四向指令、敌我目标反馈、仙术列表、奖励与升级页，不自行改动数值。探索场景把统一 `PalAudioPlayer` 注入战斗层，因此仙术 DATA 音效和普通/Boss 胜利音乐沿用系统菜单设置的音量。
