...
修改场景脚本入口、批量 EventObject 状态、音乐停止或这段主线消息时必须运行该测试。回归只保存消息编号和状态断言，不得输出原版文本；后续主线继续按真实剧情顺序扩展同类长期测试，不增加已验收的人工检查点。
...
合成测试验证固定随机序列、行动排序、双动、防御、敌我仙术、九种状态、敌我下毒/解毒、毒抗、毒杀胜负、复活、装备双击、附带物品攻击、基础使用/投掷物品、库存预留、逃跑、`R` 重复指令及资源不足降级、最低伤害、死亡目标重选、胜负、主经验升级和 `004A/0007` 三种结果分支；本地 headless 测试验证真实敌队/Sprite、60/85 个已接入/全部敌术、68/49 个使用/投掷物品脚本、首战普攻、551 号基础毒、止血草、梅花镖、逃跑、52 经验/96 文奖励及脚本 6964/6965 的阻塞请求；最后一个测试必须使用真实渲染器，验证 `R` 键入口并把敌队 17 的敌术和敌队 18 / 战场 21 的战斗、其他/物品菜单、物品/仙术动画、毒性结算、奖励与升级页截图写入被忽略的 `generated/pal/visual_tests/`。
