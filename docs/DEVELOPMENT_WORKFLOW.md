...
移动端浮动摇杆、菜单／互动键、对话、菜单和战斗点击回归：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --path . \
  --rendering-method gl_compatibility \
  --script res://tests/run_local_mobile_visual_test.gd -- --pal-mobile-controls
```

该测试必须在真实窗口中使用正式 `TileMapLayer + PalTileMapWorld`，并把 320×200 截图写入被忽略的 `generated/pal/visual_tests/`。除检查浮动摇杆只在按下后出现、松手归零外，还要用真实 `InputEventScreenTouch` 打开菜单，并在双敌人战斗中点击攻击图标和敌人 Sprite 提交指令；不能用 CPU 合成画面代替正式路径。
...
回魂仙梦南诏、水魔兽、十年前余杭与水灵珠主线回归：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --script res://tests/run_local_nanzhao_flashback_mainline_test.gd
```

该测试从场景 226 继续，覆盖两场黑苗守卫战、南诏王宫与秘道、天蛇杖、巫后脱困、两次水魔兽战、十年前余杭包袱交付、木剑商店、水灵珠交换和返回女娲神殿。敌队 102／113／315、战场 9／64／4、物品 195／292／166／265／275、强制出图续段、队伍重组和最终入口 `34711` 均有断言。修改第十五章转场、水魔兽战、剧情物品或回魂仙梦返回入口时必须运行。

圣姑救治、灵儿生产、试炼窟与傀儡虫主线回归：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --script res://tests/run_local_shenggu_trial_cave_mainline_test.gd
```

该测试从场景 202 稳定状态继续，先固定大理出口恢复，再完成两味圣药、灵儿生产和 36 只傀儡虫任务。盖罗娇真实敌队 223／战场 6／对象 `[501,490]`、傀儡虫 152 的六类掉落敌人、试炼窟场景 213–225、女娲遗迹 212、土灵珠 267 脱离及最终三人队 `[1,0,4]` 均有断言。修改第十六章转场、生产剧情、敌人战后掉落、场景 teleport 或傀儡虫交付时必须运行。

五灵珠祭坛、祭雨、地魔兽与无底深渊入口主线回归：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --script res://tests/run_local_dali_altar_battle_mainline_test.gd
```

该测试从场景 173 稳定状态继续，覆盖战时大理两场黑苗遭遇、灵儿拜母、圣灵珠开启五孔、五灵珠放置、祭雨动画、庆典归还六颗灵珠和地魔兽敌队 287／战场 36／对象 466。最终进入场景 290“无底深渊”，固定三人队、落点 `(240,1672)`、音乐 26、战场 58 和入口 `3798`。修改第十七章转场、场景入口上下文、祭坛物品脚本、有限循环或地魔兽战时必须运行。

无底深渊、南诏王宫秘道与假巫王双战主线回归：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --script res://tests/run_local_bottomless_palace_mainline_test.gd
```

该测试从场景 290 的第十八章稳定状态继续，固定 `290 → 291 → 292 → 278 → 287 → 286 → 284 → 276 → 280 → 277` 的无底深渊和王宫秘道主路径。假巫王剧情连续执行敌队 289／战场 57／对象 `[528,528]` 和敌队 222／战场 57／对象 `[462]`，覆盖消息 `434–502`、三人／双人／单人队切换、剧情 Sprite、内殿对象状态及最终对质入口 5056。修改第十八章迷宫转场、假巫王剧情、战场或队伍重组时必须运行。

拜月教主、水魔兽终战与完整结局回归：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --script res://tests/run_local_final_battle_ending_test.gd
```

该测试从假巫王战后返回场景 280 的稳定状态继续，验证最终对质入口 `4231`、消息 `503–584`、逍遥／灵儿／阿奴 `[0,1,4]` 队伍、敌队 313／战场 55／对象 546 和真实胜利结算。战后继续断言 11 段 RNG、全部音乐切换与淡出、16 个 `0096/00A4/00A5` 结局动作、`00A6` 屏幕备份及 `00A0` 统一退出的相对顺序。结局播放器像素仍须去掉 `--headless` 运行 `run_local_td001_visual_test.gd`，并实际检查被忽略目录中的 `td001_ending_0096.png` 和 `td001_ending_00a5.png`。
...
全部有效 MAP/GOP 与场景引用的 TileMapLayer 结构门禁可用 Headless 运行：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --script res://tests/run_local_tilemap_inventory_test.gd
```
...
/Applications/Godot.app/Contents/MacOS/Godot --path . \
  --script res://tests/run_local_tilemap_visual_test.gd -- \
  --pal-visual-case=dali_altar_open
```

正式 `MapExplorer` 已退役旧 CPU 地图后端和 `--pal-map-backend=legacy`。需要排查像素差异时只运行 `run_local_tilemap_visual_test.gd`；它从正式 `PalTileMapWorld` 取得场景项，再交给测试用 `PalSceneRenderer` 生成对照图，不会把 CPU 合成带回游戏运行。
