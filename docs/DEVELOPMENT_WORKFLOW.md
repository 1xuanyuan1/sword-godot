...
合成测试验证固定随机序列、行动排序、双动、防御、敌我仙术、基础使用/投掷物品、库存预留、逃跑、`R` 重复指令及资源不足降级、最低伤害、死亡目标重选、胜负、主经验升级和 `004A/0007` 三种结果分支；本地 headless 测试验证真实敌队/Sprite、58/85 个基础/全部敌术、29/49 个基础使用/投掷物品、首战普攻、止血草、梅花镖、逃跑、52 经验/96 文奖励及脚本 6964/6965 的阻塞请求；最后一个测试必须使用真实渲染器，验证 `R` 键入口并把敌队 17 的敌术和敌队 18 / 战场 21 的战斗、其他/物品菜单、物品/仙术动画、奖励与升级页截图写入被忽略的 `generated/pal/visual_tests/`。
