...
本地 `run_local_tilemap_visual_test.gd` 会让 TileMapLayer 和 CPU 使用同一会话状态。正式项目的默认清屏色固定为黑色，与 CPU 索引画布初始化及 SDLPal 地图边界语义一致；否则地图图块之间极少量未覆盖像素会露出 Godot 默认灰底。当前从余杭到蛤蟆山、白苗酒店、水仙尊王庙及长安共 47 个固定视口达到 320×200 零差异像素。
