...
正式探索已完全退役几何占位和整屏 CPU 合成，只使用 `TileMapLayer + PalTileMapWorld`。角色和事件对象会从本机导入的 `MGO.MKF` 读取原版 Sprite。地图导入阶段为每个唯一 `map_number` 生成 Godot 4.7 `TileSet` 和四个 `TileMapLayer`；CPU 索引画布只存在于测试像素基准。
...
`0015` 下发的有效剧情动作帧在 `PalTileMapWorld` 中优先于残留步态标志。正常移动会先清除旧剧情动作，因此不会破坏普通行走；移动之后再次设置的倒地、举杯或固定姿势必须由正式 TileMap 选中该帧。CPU 像素测试直接复用正式世界生成的场景项，不再维护第二套选帧路径。
...
人物场景 Sprite 统一由 `PalContentDatabase.load_player_scene_sprite()` 根据 PLAYERROLES 当前编号解析，MGO 数据只按实际 Sprite 编号缓存。渲染器不能再按角色编号长期缓存结果：`0065` 会在剧情中换装，而读档会直接恢复整组 `scene_sprite_numbers`，后者不会额外发出脚本换装信号。这样喂药、端酒、倒地等特殊造型结束或读档后，正式 TileMap 会在下一次同步立即使用正确造型。
...
共享的 `PalSceneLayout` 按 `scene.c` 的坐标、逻辑层和覆盖块扫描规则实现同一流程。正式世界把布局项转成 Y 排序 Sprite2D，测试用 `PalSceneRenderer` 才把相同布局项写进 CPU 画布。这样人物经过床沿、墙体、屋檐或其他高层地图块时，遮挡关系由地图数据决定，而不是简单地让所有人物永远位于地图上方。
...
星芒和事件对象共用基准 Y，并经过 `PalSceneLayout.expanded_draw_items()` 的覆盖块扫描，所以屋顶、树冠和柜架仍可遮住它；它不是 HUD 置顶提示。Shader 使用固定白／金／琥珀像素和 1.2 秒呼吸周期，不受日夜调色板影响。实际执行正向 `001E/001F` 后，探索控制器才把该 EventObject 记入 `GameSession.collectible_marker_event_ids` 并刷新世界；普通宝箱还会依赖原版空箱入口或隐藏状态，可重复鼠儿果则只永久熄灭提示，不改变继续采摘的原版行为。

CPU 整屏合成不新增星芒能力。像素基准测试通过 `PalTileMapWorld.set_collectible_markers_enabled(false)` 关闭辅助层，正式游戏始终默认开启。

`ScriptVM 007F` 使用独立的剧情镜头偏移。正式运行只改变 `PalCamera`，不会修改 `GameSession.viewport_position`、队伍脚底、轨迹或事件坐标；测试对照会读取同一个整数视口位置。逐帧平移、固定格坐标和复位因此不会误触碰撞，场景切换会清空临时偏移。

`run_local_tilemap_inventory_test.gd` 会在 Headless 下遍历全部 223 份有效地图和 221 个场景引用，确认都能由正式 `PalTileMapWorld` 载入。本地 `run_local_tilemap_visual_test.gd` 则让 TileMapLayer 和 CPU 测试基准使用同一会话状态与 `PalSceneLayout`；正式项目的默认清屏色固定为黑色，与 CPU 索引画布初始化及 SDLPal 地图边界语义一致。当前从余杭到蜀山、锁妖塔、圣姑住处、神木林、大理、火麒麟洞、女娲神殿、回魂仙梦南诏、十年前余杭、试炼窟女娲遗迹、五灵珠祭坛、祭雨、无底深渊和南诏王宫终战共登记 149 个固定视口，本次退役验收已在 Godot 4.7 Metal 真窗口下逐项达到 320×200 零差异，并实际检查祭坛和最终对质等关键画面。离屏 SubViewport 会等待 `RenderingServer.frame_post_draw`；Metal 长时间连续运行停止提交时，可在 `--` 后追加 `--pal-visual-case=用例名` 逐项隔离，不能用 dummy renderer 代替。
