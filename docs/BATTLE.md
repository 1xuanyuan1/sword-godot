...
样板使用新的临时 `GameSession`，不会污染探索进度。它已使用 `PalBattleController` 真实执行攻击、防御、敌人物理攻击、自动防御、体力扣减、死亡和胜负，不伪造结果。`PalBattleUI` 按 `uibattle.c` 固定坐标绘制角色状态框、头像、HP/MP 数字、当前角色箭头和四向图标，并固定在所有按脚底 Y 排序的战斗人物之上；仙术窗口等大面板不会被人物反向盖住。选敌时直接让敌人 Sprite 按调色板索引低四位 `+7` 闪烁，不显示自制目标 HP 或编号。
