...
Godot 版以固定 SDLPal 的经典回合制路径为行为基准，不启用 `ENABLE_REVISIED_BATTLE` 蓄力模式。静态资源、普攻回合、首批玩家/敌人攻击仙术、基础使用/投掷物品、逃跑、主经验成长和 `ScriptVM 004A/0007` 剧情桥接已经接通；毒与异常状态仍在开发。
...
画面按 `PAL_BattleShowEnemyMagicAnim()` 先让敌人向右下蓄势，播放敌人施法音效和施法帧，再以原版单体/三人/整体/战场锚点播放 FIRE Sprite；生效后显示蓝色伤害、角色受击变色和逐段后退。当前本地数据 85 个带敌术的敌人中，58 个无额外脚本的基础攻击敌术可完整结算；敌队 17 的敌术对象 312 已通过真实逻辑和 OpenGL 动画回归。其余依赖中毒、状态、召唤或成功脚本的 27 个敌术会明确提示尚未支持，不会退化成普攻或凭空扣血。
...
- 使用物品支持由 `001B/001C/001D` 直接增减 HP/MP 的基础恢复脚本，播放原版物品名称、角色帧 5、音效 28、目标变色和黄／青恢复数字；
- 投掷物品支持 `0042` 模拟仙术、`0021` 固定伤害和 `0066` 武器伤害，使用 FIRE 特效、角色帧 5/6、原版投掷音效和蓝色伤害数字；
- 毒、复活、异常状态、隐身和特殊条件物品仍显示为不可用，不会只扣库存而漏掉效果。

本地 DOS 数据现有 29 个基础使用物品与 49 个基础投掷物品可以走完整结算路径；止血草和梅花镖已通过真实脚本、库存和 OpenGL 动画回归。
...
- 敌人状态/脚本仙术、召唤、合击、特殊物品脚本和自动战斗；
- 中毒、异常状态、装备双击/特殊毒效果、保护与替队员承伤；
- 隐藏经验成长、偷取/掉落及敌人战后脚本；
- 状态类仙术、完整死亡／复活动画和持续回合效果；
- 自动战斗、状态页和战斗中脚本事件的完整执行。
...
截图只写入 `generated/pal/visual_tests/`。当前真实资源验收覆盖 154 条敌人属性、380 个敌队、65 个战场定义、43 个脚本引用战场、六名角色战斗 Sprite、55 组 FIRE 仙术 Sprite，以及敌队 17 的敌术 312、敌队 18 / 战场 21 的气疗术、风咒、普攻、52 经验/96 文奖励页和李逍遥升级页。
