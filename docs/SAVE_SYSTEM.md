...
三个榜位在排行榜页顶部按同级标签排列，可用键盘左右键或直接点击标签切换。Toy 试玩地址没有正式 ID 时，切换仍立即更新当前标签并返回试玩提示，不会因无效 SDK 请求长期停留在忙碌状态；正式页面再读取对应真实榜单。
