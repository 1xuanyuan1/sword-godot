...
当前可启动资源实验室，选择本机合法取得的 `Data` 目录后执行只读校验和本地导入；已有生成内容时可直接选择“开始新游戏”，或从“读取存档”打开与游戏内一致的原版 100 槽界面并恢复进度。源码工程的生成内容位于 `res://generated/pal/`，桌面发布包则位于可写的 `user://generated/pal/`，两者都不提交 Git。导入器会为每个有效 `map_number` 生成 Godot 4.7 `TileSet + TileMapLayer`，离线转换剧情实际引用的 RIX/OPL 与 VOC，并只安装压缩 `RNG.MKF`；过场在运行时按动画分块逐帧解码到一张索引画布，不再默认生成或一次性载入全部 RGBA PNG。探索场景只使用原生 TileMap、Camera2D、Sprite2D 和 AudioStreamPlayer；整屏 CPU 合成仅保留在测试像素对照中。地图探索已经支持双层地图、MGO 角色/NPC、原版方向步态、遮挡排序、移动阻挡、按朝向扫描 PAL half 格的搜索事件、接触事件、EventObject 自动路线与追逐、临时隐藏/重现、场景 BGM 与剧情音效。首场景进入脚本可完整执行，原版中文消息会配合 RGM 角色肖像，在上、下或中部对话框中按空格/回车推进；用引号标记的系统叙述会显示为较小字号的居中黑底白字 Toast。`0036/0037` 会在 HUD 全屏播放对应 RNG 剧情动画并阻塞脚本，结束后继续执行角色数值与习得仙术等后续指令。探索时按 Esc、M 或 Tab 打开原版布局菜单，按 I 直接打开原版物品选择页，按 F10 返回资源实验室；经典菜单的状态页使用原版 FBP、RGM 头像、装备图标和属性坐标，场外仙术页支持施法者、3×5 仙术列表、MP/说明、单体/全体目标及真实使用/成功脚本，成功后才扣除 MP；物品“装备／使用”均已可用，装备页支持六槽初始装备、角色许可、背包换装和实时属性，剧情商店支持买入/卖出、确认与金钱/库存结算，“系统”页可独立调节音乐和音效音量。战斗已解析敌人、敌队、战场、站位和双方原版 Sprite，并实现经典普攻、普通/召唤合击、玩家/敌人攻击与状态仙术、物品、毒与九种经典状态及逃跑闭环：固定 SDLPal 随机序列、全队指令、身法队列、双动敌人、敌人物理 AI、伤害、恢复、死亡和胜负；李逍遥的头巾、披风、布袍、木剑、草鞋和护腕等初始装备会在战斗前执行原版装备脚本，攻击、防御、身法、毒抗、五灵抗性、攻击全体、装备双击、合击覆盖和战斗 Sprite 覆盖不再遗漏。画面已改用官方角色状态框、攻击／仙术／合击／其他四向图标、其他与物品菜单、敌我目标箭头/闪烁、上浮数字和双方逐帧动作。双方普通攻击已读取 PLAYERROLES 与敌人 DATA 原版音效。仙术会按真实名称选择目标、扣除 MP，气疗术等恢复法术执行原版成功脚本，五灵咒按敌人抗性与战场修正结算；风神等召唤仙术使用 F.MKF 神将与后续 FIRE 特效，梦蛇会对施法者自己执行临时 Sprite 变身。下毒、解毒、复活、混乱、定身、昏睡、封咒、傀儡、勇气、防护、加速和双击会按经典回合执行、递减并显示毒性结算。右侧合击按健康队员数量启用，读取角色/装备的真实合击仙术，让每名贡献者消耗仙术 MP 数值对应的 HP（最低保留 1），再以合计武术与灵力结算并播放原版演出；濒死或异常角色也会在自动防御成功时由 PLAYERROLES 指定的健康队友上前保护。当前数据 85/85 个敌术均走完整结算路径；敌人物理攻击配置的附带物品毒也已接通。敌人的 `script_on_turn_start`、`script_on_ready` 和 `script_on_battle_end` 由专用战斗上下文执行，支持战斗内对白/音效、动态换仙术、血量分支、敌人逃跑、召唤、分裂、变身及随机掉落；真实静态敌队的 27/22/10 个对应入口均通过可达指令审计。战斗物品页使用原版 3×7 背包布局，84 个使用物品脚本和 49 个投掷物品脚本落在当前完整路径，支持恢复、投掷、临时属性/Sprite、隐藏、偷取、收妖、吹飞、召唤与变身；逃跑会按原版公式成功、失败或被 Boss 禁止，并把结果 3 交回剧情分支。胜利后会播放普通/Boss 胜利音乐，按敌人属性结算经验与金钱，执行敌人战后掉落脚本、主等级随机成长、升级回满、升级习得仙术和经典战后半恢复，并以原版窗口显示奖励及升级前后数值。资源实验室的“战斗样板”会补满两名队员 HP/MP，并预置止血草、鼠儿果、梅花镖和血玲珑，便于直接验证仙术、合击与物品。`ScriptVM 004A/0007` 已能暂停探索、切换战斗 BGM、覆盖打开同一 `GameSession` 的战斗，并按胜利/战败/逃跑入口恢复剧情；隐藏经验成长、召唤／变身、偷取／收妖、动态敌人脚本及主线所需死亡／复活流程均已进入真实结算；少数动作细节继续作为发布前视觉优化，不再阻塞主线。真实资源自动回归已按连续章节边界覆盖到无底深渊、南诏王宫、假巫王双战、拜月教主／水魔兽终战和完整 DOS 结局。金凤凰蛋壳 275 现由回魂仙梦返回后的真实脚本 `34687` 正式发放，不提前伪造。资源实验室中的“剧情测试”只保留桂花酒流程相关的待验收入口；完成项从人工测试界面移除，行为继续由自动回归测试覆盖。

正式 `MapExplorer` 已退役隐藏整屏 CPU 地图合成、RGBA 纹理上传和 legacy 后端参数，只保留 `TileMapLayer + PalTileMapWorld`。CPU 地图／场景渲染器仅供测试像素对照，和正式世界共用 `PalSceneLayout` 的基准 Y 与特殊遮挡规则。

Android、iOS 和检测到触屏的 Web/桌面环境会自动启用移动操作：自由探索时左上角固定“菜单”、右下角固定“互动”，手指在其余地图区域按下才显示浮动摇杆，拖动移动、松手立即消失；进入剧情、菜单、战斗或转场会清空摇杆状态。对话可点击补全文字并再次点击推进，标题、存档和经典菜单可直接点击条目，菜单右上角提供“返回”；战斗可直接点击四向指令、仙术／物品列表以及敌我目标。键盘操作保持不变，桌面视觉回归可追加 `--pal-mobile-controls` 强制显示触控层。

系统菜单现提供 100 个版本化 Godot 存档槽，保存完整队伍、装备、背包、场景、EventObject 和脚本游标状态，并使用格式版本、PAL 内容指纹与 SHA-256 诊断不兼容或损坏文件。槽位页显示中文地点、保存时间、队伍头像、姓名和等级；读档不会重跑场景进入脚本。该格式不兼容原版 `.rpg`。Toy Web 发布页在 SDK 可用时额外提供一个跨设备云存档槽位，以 gzip、Base64、分块清单和双重校验安全覆盖指定本地槽；同时提供李逍遥等级、当前队伍等级总和与持有金钱三个历史最高总榜。

真实资源回归已按连续章节边界覆盖从御剑教学、水月宫惨案、离开余杭、苏州、林家堡夜间、隐龙窟、白河村药材任务、第七章、鬼阴山、扬州、蛤蟆山、长安尚书府、蜀山锁妖塔、圣姑住处、神木林、隐密树洞、大理族议、火麒麟、回魂仙梦、灵儿生产、试炼窟、五灵珠祭坛、祭雨和地魔兽，一直推进到无底深渊、南诏王宫、假巫王双战、拜月教主／水魔兽终战及结局统一退出。最终章固定敌队 289／222／313、战场 57／55、完整消息 `434–584`、11 段 RNG 和 16 个结局播放器动作；全部 223 份有效地图和 221 个场景引用均可由正式 TileMapLayer 路径载入，149 个固定视口逐项通过 Godot 4.7 Metal 真窗口 320×200 零差异检查并实际验图。当前主线脚本和音画请求链已可验证到最终 `00A0`，RNG 全部 12 段／1464 帧也已通过运行时流式解码；macOS Universal 发布包已完成真实首次导入与运行验收，Windows x86_64 发布包已完成交叉导出和内容审计，剩余工作仅为可选增强。
...
安装 Godot 4.7 对应平台的发布模板后，可在仓库根目录生成两个不含原版资源的桌面包：

```bash
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --export-release "macOS Desktop" builds/desktop/SwordGodotStudyPort-macOS.zip
/Applications/Godot.app/Contents/MacOS/Godot --headless --path . \
  --export-release "Windows Desktop" builds/desktop/SwordGodotStudyPort-Windows-x86_64.exe
```

发布包明确排除 `Data/`、`generated/`、测试、存档和其他原版派生内容。玩家首次启动会进入资源实验室，仍须自行提供合法取得的 `Data`；导入结果写入 `user://generated/pal/`，不会尝试写入只读的应用包。文本与 RIX 转换需要目标机器安装 Python 3 和 C++17 编译器，并准备官方 SDLPal 源码：设置 `SDLPAL_DIR`，或把名为 `sdlpal-official` 的源码目录放在应用／EXE 旁边。发布包会把所需的 4 个辅助源码释放到 `user://pal_import_tools/` 后再调用这些本机工具。

桌面版的默认用户数据位置为：

- macOS：`~/Library/Application Support/Godot/app_userdata/Sword Godot Study Port/`
- Windows：`%APPDATA%\Godot\app_userdata\Sword Godot Study Port\`
